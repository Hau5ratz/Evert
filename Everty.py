...
class N_weekly(N):
    ...
    def create(self):
        ...
        new_record = []
        forms = []
        forms += ['=MAX(0,%s-INDIRECT(CONCAT(RIGHT($I$1,1),ROW())))']
        forms += ['1-(I%s/H%s)']
        forms += ["IF(ISERROR(VLOOKUP(E%s,'%s'!$A$2:$B$22155,2,FALSE)),0,VLOOKUP(E%s,'%s'!$A$2:$B$22155,2,FALSE))"]
        forms += ["IF(M%s-L%s=0,0,IF($I%s/((M%s-L%s)/(M$1-L$1))<0,0,($I%s/((M%s-L%s)/(M$1-L$1)))))"]
        forms += ['=IF(P2-O2>100,"Yes","No")']
        c = 2
...
class Ndstruct(N):
    def __init__(self, obj, **kwarg):
        N.__init__(self, obj, **kwarg)
        print('Loading wb..')
        self.wb = openpyxl.load_workbook(self.obj)
